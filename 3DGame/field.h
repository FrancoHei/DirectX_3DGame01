...
private:
	...
	bool					isMove			= false;
public:
	Field(FIELD  field_, D3DXVECTOR3 pos_, int numFieldX, int numFieldY) : field(field_)
	{
		CreateField(pos_, D3DXVECTOR3(0.0f, 0.0f, 0.0f), numFieldX, numFieldY, FIELDSIZE, FIELDSIZE);
	}
	...
	void CreateField(D3DXVECTOR3 pos_, D3DXVECTOR3 rot_, int numBlockX, int numBlockZ, float sizeX, float sizeZ)
	{
		// 位置・回転・スケールの初期設定
		pos = pos_;
		rot = rot_;
		scl = D3DXVECTOR3(1.0f, 0.1f, 1.0f);
